...
function FormSignUp() {
  ...
  const handleRegister = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      setSnackbar({
        open: true,
        message: response.data?.msg || "Register Berhasil",
        severity: "success",
      });
      resetForm(); // kosongkan form setelah berhasil
    } catch (err) {
      const msg = err.response?.data?.msg || "Register gagal";
  ...
  return (
    <>
      <span className="mt-6 flex justify-center font-bold">Create an Account</span>
          ...
          {({ isSubmitting }) => (
            <Form>
              <div className="mb-6">
                <Field name="name">
                  {({ field }) => (
                    <LabeledInput {...field} id="name" type="text" label="Name" placeholder="Barick Muhammad Dziellan" />
                  )}
                </Field>
              ...
              <div className="mb-6">
                <Field name="email">
                  {({ field }) => (
                    <LabeledInput {...field} id="email" type="email" label="Email Address" placeholder="hello@example.com" />
                  )}
                </Field>
              ...
              <div className="mb-6">
                <Field name="password">
                  {({ field }) => (
                    <LabeledInput {...field} id="password" type="password" label="Password" placeholder="●●●●●●●●●●●●●●" />
                  )}
                </Field>
      ...
      {/* Divider */}
      <div className="py-9 px-7 flex flex-col justify-center items-center text-xs text-gray-03 relative">
      ...
      <div className="flex justify-center text-sm">
        <span className="text-gray-03">Already have an account?{" "}</span>
